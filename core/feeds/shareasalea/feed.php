...
class PShareASaleAFeed extends PCSVFeed
{
    ...
    function formatProduct($product)
    {
        ...
        if (!isset($this->merchant_id)) {
            $this->addErrorMessage(9000, 'MerchantID not configured. Need advanced command: $merchant-id = ....',false,$product->attributes['title']);
            $this->productCount--;
            //$this->merchant_id = '';
        }
        if (!isset($this->merchant)) {
            $this->addErrorMessage(9001, 'Merchant not configured. Need advanced command: $merchant = ....',false,$product->attributes['title']);
            $this->productCount--;
            //$this->merchant = '';
        }
