...
//optional
        ...
        $this->addAttributeMapping('item_group_id', 'Item Group ID', true);
        $this->addAttributeMapping('regular_price', 'Original Price', true);
        $this->addAttributeMapping('weight', 'Ship Weight', true); //weight in pounds
...
//Availability
        if ($product->attributes['stock_status'] == 1) //RC 'woocommerceStock' rule changes stock_status to 'In Stock', so all products may be 'Out of Stock'
...
//Weight in lbs
        ...
        switch ($product->attributes['weight_unit']) {
            ...
            case 'oz':
                ...
                break;
            default: //already in lbs
                $weight_multiplier = 1;
                break;
        }
        if ($product->attributes['weight'] > 0)
            $product->attributes['weight'] = sprintf('%0.4f', $product->attributes['weight'] * $weight_multiplier);
